-- Fix dossiers data exposure: Clercs should not see sensitive client data
-- Strategy: Update RLS so clercs can only access via a secure view that masks data

-- First, update the SELECT policy on dossiers to exclude direct clerc access
-- Clercs will need to use the dossiers_secure view instead
DROP POLICY IF EXISTS "Users can view assigned dossiers" ON public.dossiers;

-- Admins and collaborateurs can see all dossiers directly
-- Creators can see their own dossiers
-- Clercs must use the secure view
CREATE POLICY "Full access for admins collaborateurs and creators" 
ON public.dossiers 
FOR SELECT 
TO authenticated
USING (
...
-- Create secure view that masks sensitive data for clercs
-- Admins and collaborateurs see all data, clercs see masked data
